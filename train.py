...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.mode == 'TXT':
            ...
            train_img_ids, val_img_ids = load_dataset(args.root, args.dataset, args.split_method)
    ...
    def testing (self, epoch):
        ...
        with torch.no_grad():
            for i, ( data, labels) in enumerate(tbar):
                ...
                self.metric.update(labels.cpu(), pred.cpu())#new add
                miou, prec, recall, fmeasure = self.metric.get()
                
                ture_positive_rate, false_positive_rate, recall, precision = self.ROC.get()
                _, mean_IOU = self.mIoU.get()
                tbar.set_description('Epoch %d, test loss %.4f, mean_IoU: %.4f' % (epoch, losses.avg, miou ))
            test_loss=losses.avg
        # save high-performance model
        save_model(miou, self.best_iou, self.save_dir, self.save_prefix,
